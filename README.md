...
- 自动识别 B 站视频链接（`BV`/`AV` 号）、音频链接（`bilibili.com/audio/au...`）、专栏链接（`bilibili.com/read/cv...`）、直播链接（`live.bilibili.com/...`）、番剧链接（`bilibili.com/bangumi/play/ep...`、`ss...`）、动态链接（`bilibili.com/opus/...`、`t.bilibili.com/...`）、UP 主主页链接（`space.bilibili.com/...`）、`b23.tv` 短链和 QQ 小程序分享。
- 一条消息里出现多个 B 站链接时按顺序逐个解析（`max_links_per_message` 控制，默认 3）；超出上限时会明确提示剩余条数，不会静默丢弃。
- 封面、头像、二维码会先完整下载，再渲染成长图，避免图片“还在加载就被截图”。
- 长图下方附带 B 站源链接；开启下载后，只在用户发送完整编号时下载视频/音频。
- 视频卡片可显示热门评论、评论头像、昵称和回复；多 P 视频额外列出分 P 清单（序号、标题、时长，最多 10 P）。
- UP 主主页会生成用户名片：昵称、头像、认证、等级、粉丝、关注、投稿数、获赞数和个人签名。
- 可选使用 AstrBot 已保存的模型商生成 AI 总结：视频（字幕/语音转写）、专栏（正文）、动态（正文）。同一内容的概要会在 `summary_cache_seconds` 有效期内复用，不会反复调用模型。
- AI 总结支持 B 站公开字幕、登录后的 AI 字幕，以及可选的 `faster-whisper` 语音转写。
- 也可以不为所有链接开启自动总结：打开 `enable_manual_summary` 后，发送“视频总结 BV号或链接”只总结指定的那一个视频。
- 直播查询命令可随时查看任意直播间状态；开播/下播提醒支持订阅直播间，主播开播或下播时自动通知，开播通知会附带完整直播卡片。
...
`astrbot_plugin_bilibili_media_parser-v1.4.0.zip`
...
| 只想按需总结 | `enable_manual_summary` | 打开后发送“视频总结 BV号或链接”，无需全局开启 |
...
生成的概要在 `summary_cache_seconds`（默认 86400 秒，即 1 天）内按内容编号缓存，同一视频被反复分享时不会重复抓字幕或调用模型；填 0 关闭缓存。

### 只总结指定内容

如果不想为群里所有链接都开启自动总结，可以只打开 `enable_manual_summary`，然后发送：

```text
视频总结 BV1xx411c7RD
视频总结 https://b23.tv/xxxx
```

插件只总结这一条并回复文字概要，其余链接照常只发解析卡片。命令关键词可用 `manual_summary_keyword` 修改；未开启该开关时，这几个字不会被插件拦截。目前只支持视频，专栏和动态在自动总结开启时本来就会生成概要。
...
主播开播时，插件会自动向订阅的会话推送通知，并附带一张完整的直播间卡片（封面、主播、分区、在线人数、二维码）；如果卡片渲染失败则自动退回纯文本提醒。订阅保存在本地文件中，重启不丢失；为避免刷屏，插件重启后已在播的直播间不会立刻触发提醒，只有观察到“未开播 → 开播”的变化才通知。轮询任务在插件加载时即启动，不再依赖群里恰好有新消息。
...
- UP 主主页：`space.bilibili.com/{uid}` 会生成用户名片，展示昵称、认证、等级、粉丝、关注、投稿数、获赞数与个人签名。带 BV 号的空间链接仍按视频解析；合集、相册等空间子页没有稳定含义，仍然忽略。
- 多链接：一条消息里出现多个 B 站链接时按出现顺序逐个解析，最多 `max_links_per_message`（默认 3，上限 5）个；超出的条数会明确提示，不会静默丢弃。
...
- `summary_cache_seconds`：概要缓存秒数，默认 86400（1 天），填 0 关闭。
- `enable_manual_summary`：是否启用“视频总结”命令，默认关闭。
- `manual_summary_keyword`：手动总结命令前缀，默认“视频总结”，留空关闭。

...
- 直播查询命令可随时查看任意直播间状态；开播提醒支持订阅直播间，主播开播自动通知。
...
`astrbot_plugin_bilibili_media_parser-v1.2.0.zip`
...
确认 `enable_live_monitor` 已开启且命令使用的是真实房间号（短号会自动换算）。插件只通知“未开播 → 开播”的变化，重启后已在播的直播间要等下次下播再开播才会提醒。
